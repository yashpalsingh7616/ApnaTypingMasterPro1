...
// ── Session save/load (localStorage — persists across tabs & restarts,
//    same as any normal "stay logged in" experience) ──────────
window.saveSession = function(){
  try {
    localStorage.setItem('atm', JSON.stringify({
...
window.loadSession = function(){
  try {
    const s = JSON.parse(localStorage.getItem('atm')||'{}');
...
// ── Admin session save/load — previously admLoggedIn was a plain in-memory
//    variable with NO persistence at all, so the admin had to re-enter their
//    password on every single page reload/navigation. Persist it the same
//    way the regular user session is persisted. Note: this only gates which
//    UI the admin SEES (login form vs panel) — every real write still goes
//    through Firebase Auth + server-side rules checked against the admin's
//    actual email, so this local flag being present doesn't grant any real
//    access on its own. ──────────
window.saveAdmSession = function(email){
  try { localStorage.setItem('atm_admin', JSON.stringify({ admLoggedIn:true, email:email||'' })); } catch(e){}
};
window.clearAdmSession = function(){
  try { localStorage.removeItem('atm_admin'); } catch(e){}
};
window.loadAdmSession = function(){
  try {
    const s = JSON.parse(localStorage.getItem('atm_admin')||'{}');
    return !!s.admLoggedIn;
